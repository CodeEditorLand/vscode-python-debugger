...
suite("Debugging - Outdated Debugger Prompt tests.", () => {
	...
	[someRequest, someEvent, someOutputEvent].forEach((message) => {
		test(`Don't show prompt when non-telemetry events are seen: ${JSON.stringify(
			message
		)}`, async () => {
			showInformationMessageStub.returns(Promise.resolve(undefined));
			...
			const session = createSession();
			const prompter =
				await promptFactory.createDebugAdapterTracker(session);
			assert(prompter);
