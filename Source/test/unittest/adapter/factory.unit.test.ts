...
import {
	...
	debugStateKeys,
} from "../../../extension/debugger/adapter/factory";
...
} from "../../../extension/common/persistentState";
...
suite("Debugging - Adapter Factory", () => {
	...
	setup(() => {
		...
		getActiveEnvironmentPathStub = sinon.stub(
			pythonApi,
		...
		when(
			stateFactory.createGlobalPersistentState<boolean | undefined>(
	...
	test("Display a message if no python interpreter is set", async () => {
		...
		await expect(promise).to.eventually.be.rejectedWith(
			DebugConfigStrings.debugStopped
		);
	...
	test('Return Debug Adapter executable if request is "attach", and listen is specified', async () => {
		const session = createSession({
			request: "attach",
			listen: { port: 5678, host: "localhost" },
		});
		const debugExecutable = new DebugAdapterExecutable(pythonPath, [
			debugAdapterPath,
		]);
	...
	test("Send attach to local process telemetry if attaching to a local process", async () => {
		...
		assert.ok(
			Reporter.eventNames.includes(
				EventName.DEBUGGER_ATTACH_TO_LOCAL_PROCESS
			)
		);
	});
	...
	test('Use "debugAdapterPython" when specified', async () => {
		...
		const debugExecutable = new DebugAdapterExecutable("/bin/custompy", [
			debugAdapterPath,
		]);
		...
		resolveEnvironmentStub
			.withArgs("/bin/custompy")
			.resolves(customInterpreter);
		const descriptor = await factory.createDebugAdapterDescriptor(
			session,
			nodeExecutable
		);
