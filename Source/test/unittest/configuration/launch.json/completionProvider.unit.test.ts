...
suite("Debugging - launch.json Completion Provider", () => {
	...
	test("Cannot provide completions for non launch.json files", () => {
		...
		document
			.setup((doc) => doc.uri)
			.returns(() => Uri.file("settings.json"));
		assert.strictEqual(
			LaunchJsonCompletionProvider.canProvideCompletions(
				document.object,
				position
	...
	function testCanProvideCompletions(
		position: Position,
		offset: number,
		json: string,
    ...
    "configurations": [
		...
		testCanProvideCompletions(
			position,
			json.indexOf("# Cursor Position"),
			json,
			true
		);
	});
	...
	test("No Completions for files ending with launch.json", async () => {
		...
		document
			.setup((doc) => doc.uri)
			.returns(() => Uri.file("x-launch.json"));
		const { token } = new CancellationTokenSource();
		const position = new Position(0, 0);
	...
	test("Get Completions", async () => {
		...
		document
			.setup((doc) => doc.offsetAt(typemoq.It.isAny()))
			.returns(() => json.indexOf("# Cursor Position"));
		const position = new Position(0, 0);
		const { token } = new CancellationTokenSource();
