...
import {
	...
	MultiStepInput,
} from "../../../extension/common/multiStepInput";
import {
...
suite("Debugging - Configuration Service", () => {
	...
	class TestPythonDebugConfigurationService extends PythonDebugConfigurationService {
		public static async pickDebugConfiguration(
			...
			return PythonDebugConfigurationService.pickDebugConfiguration(
				input,
				state
			);
		}
	...
	test("Should use attach resolver when passing attach config", async () => {
		...
		attachResolver
			...
			.returns(() =>
				Promise.resolve(
					expectedConfig as unknown as AttachRequestArguments
				)
			)
	...
	[{ request: "launch" }, { request: undefined }].forEach((config) => {
		test(`Should use launch resolver when passing launch config with request=${config.request}`, async () => {
			...
			launchResolver
				.setup((a) =>
					a.resolveDebugConfiguration(
						typemoq.It.isValue(folder),
						typemoq.It.isValue(
							config as DebugConfiguration as LaunchRequestArguments
						),
						typemoq.It.isAny()
					)
				)
				.returns(() =>
					Promise.resolve(
						expectedConfig as unknown as LaunchRequestArguments
					)
				)
				.verifiable(typemoq.Times.once());
			attachResolver
			...
			const resolvedConfig =
				await configService.resolveDebugConfiguration(
					folder,
					config as DebugConfiguration
				);
	...
	test("Picker should be displayed", async () => {
		...
		const multiStepInput =
			typemoq.Mock.ofType<MultiStepInput<DebugConfigurationState>>();
		multiStepInput
			.setup((i) => i.showQuickPick(typemoq.It.isAny()))
			.returns(() => Promise.resolve(undefined))
			.verifiable(typemoq.Times.once());

		await TestPythonDebugConfigurationService.pickDebugConfiguration(
	...
	test("Ensure generated config is returned", async () => {
		...
		multiStepFactory
			...
			.verifiable(typemoq.Times.once());
		TestPythonDebugConfigurationService.pickDebugConfiguration = (
			_,
			state
		) => {
			Object.assign(state.config, expectedConfig);
	...
	test("Ensure `undefined` is returned if QuickPick is cancelled", async () => {
		...
		multiStepFactory
			...
			.returns(() => multiStepInput as MultiStepInput<unknown>)
			.verifiable(typemoq.Times.once());
		const config = await configService.resolveDebugConfiguration(
			folder,
	...
	test("Use cached debug configuration", async () => {
		...
		launchResolver
			...
			.returns(() =>
				Promise.resolve(expectedConfig as LaunchRequestArguments)
			)
