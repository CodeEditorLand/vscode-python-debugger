...
import { BaseConfigurationResolver } from "../../../../extension/debugger/configuration/resolvers/base";
...
suite("Debugging - Config Resolver", () => {
	...
	> {
		...
		public debugOption(
			...
			return BaseConfigurationResolver.debugOption(
				debugOptions,
				debugOption
			);
		}
		...
		public isDebuggingFastAPI(
			debugConfiguration: Partial<
				LaunchRequestArguments & AttachRequestArguments
			>
		) {
			return BaseConfigurationResolver.isDebuggingFastAPI(
				debugConfiguration
			);
		}
		...
		public isDebuggingFlask(
			...
			return BaseConfigurationResolver.isDebuggingFlask(
				debugConfiguration
			);
		}
	}
	...
	[
		...
		{
			title: "Should get directory of active program when there are 0 workspace folders",
			workspaceFolders: [],
		},
	].forEach((item) => {
		test(item.title, () => {
	...
	test("Do nothing if debug configuration is undefined", async () => {
		await resolver.resolveAndUpdatePythonPath(
			undefined,
			undefined as unknown as LaunchRequestArguments
		);
	});
	test("python in debug config must point to pythonPath in settings if pythonPath in config is not set", async () => {
		...
		getInterpreterDetailsStub.resolves({
			path: [pythonPath],
	...
	test("config should not change debugAdapterPython and debugLauncherPython if already set", async () => {
		...
		const config = {
			debugAdapterPython: debugAdapterPythonPath,
			debugLauncherPython: debugLauncherPythonPath,
		};
		const pythonPath = path.join("1", "2", "3");
		...
		expect(config).to.have.property(
			"debugAdapterPython",
			debugAdapterPythonPath
		);
		expect(config).to.have.property(
			"debugLauncherPython",
			debugLauncherPythonPath
		);
	});
	...
	test("Is debugging fastapi=true", () => {
		...
		const isFastAPI = resolver.isDebuggingFastAPI(
			config as LaunchRequestArguments
		);
	...
	test("Is debugging flask=true", () => {
		...
		const isFlask = resolver.isDebuggingFlask(
			config as LaunchRequestArguments
		);
