...
import * as remoteAttach from "../../../../extension/debugger/configuration/providers/remoteAttach";
...
suite("Debugging - Configuration Provider Remote Attach", () => {
	...
	test("Launch JSON with default host name", async () => {
		...
		const configurePort = await remoteAttach.buildRemoteAttachConfiguration(
			instance(input),
			state
		);
		if (configurePort) {
