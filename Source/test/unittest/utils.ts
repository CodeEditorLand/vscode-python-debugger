...
class DebuggerSession {
	...
	public async waitForBreakpoint(
		bp: vscode.Breakpoint,
		opts: { clear: boolean } = { clear: true }
	) {
		...
		if (opts.clear) {
			...
			await this.raw!.customRequest("continue", {
	...
	public handleDAPMessage(
		_src: DAPSource,
		baseMsg: DebugProtocol.ProtocolMessage
	) {
...
export class DebuggerFixture extends PythonFixture {
	public resolveDebugger(
		...
		debuggers.setDAPHandler(id, (src, msg) =>
			session.handleDAPMessage(src, msg)
		);
		return session;
	}
