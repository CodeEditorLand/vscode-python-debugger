	...
	implements IDebugAdapterDescriptorFactory
{
	...
	public async createDebugAdapterDescriptor(
		...
		if (command.length !== 0) {
			...
			const logArgs = configuration.logToFile
				? ["--log-dir", EXTENSION_ROOT_DIR]
				: [];
	...
	private async getDebugAdapterPython(
		...
		if (interpreter?.path) {
			traceVerbose(
				`Selecting active interpreter as Python Executable for DA '${interpreter.path[0]}'`,
			);
			return this.getExecutableCommand(
				await resolveEnvironment(interpreter.path[0]),
			);
		}
	...
	private async showDeprecatedPythonMessage() {
		...
		const selection = await showErrorMessage(
			l10n.t(
				"The debugger in the python extension no longer supports python versions minor than 3.8.",
			),
			{ modal: true },
			...prompts,
		);
	...
	private async getExecutableCommand(
		interpreter: ResolvedEnvironment | undefined,
	): Promise<string[]> {
		if (interpreter) {
			if (
				(interpreter.version?.major ?? 0) < 3 ||
				((interpreter.version?.major ?? 0) <= 3 &&
					(interpreter.version?.minor ?? 0) <= 7)
			) {
