...
export class DynamicPythonDebugConfigurationService
	implements IDynamicDebugConfigurationService
{
	...
	public async provideDebugConfigurations(
		...
		const djangoManagePath =
			await DynamicPythonDebugConfigurationService.getDjangoPath(folder);
		if (djangoManagePath) {
			providers.push({
		...
		const flaskPath =
			await DynamicPythonDebugConfigurationService.getFlaskPath(folder);
		if (flaskPath) {
			providers.push({
		...
		let fastApiPath =
			await DynamicPythonDebugConfigurationService.getFastApiPath(folder);
		if (fastApiPath) {
