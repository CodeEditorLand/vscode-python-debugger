...
export async function pickDjangoPrompt(
	...
	const selection = await input.showQuickPick<
		QuickPickType,
		IQuickPickParameters<QuickPickType>
	>({
		...
		onDidTriggerItemButton: async (
			e: QuickPickItemButtonEvent<QuickPickType>,
		) => {
			if (e.item && "filePath" in e.item) {
				await window.showTextDocument(e.item.filePath, {
					preview: true,
				});
			}
		},
...
export function parseManagePyPath(
