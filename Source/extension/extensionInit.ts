...
} from "./debugger/configuration/launch.json/completionProvider";
...
export async function registerDebugger(
	...
	context.subscriptions.push(
		registerCommand(Commands.ReportIssue, () => openReportIssue()),
	);
	...
	context.subscriptions.push(
		registerCommand(
			Commands.Debug_Using_Launch_Config,
			async (file?: Uri) => {
				traceLog(
	...
	const persistentState = new PersistentStateFactory(
		context.globalState,
	...
	context.subscriptions.push(
		registerCommand(Commands.PickArguments, () => {
			return window.showInputBox({
				title: pickArgsInput.title,
				prompt: pickArgsInput.prompt,
			});
		}),
	);
	...
	context.subscriptions.push(
		debug.onDidStartDebugSession((debugSession) => {
			const shouldTerminalFocusOnStart = getConfiguration(
				"python",
	...
	context.subscriptions.push(
		workspace.registerPortAttributesProvider(
			{
				commandPattern:
					/extensions.ms-python.debugpy.*debugpy.(launcher|adapter)/,
			},
	...
	context.subscriptions.push(
		workspace.onDidChangeConfiguration(
			(event: ConfigurationChangeEvent) => {
				if (
					event.affectsConfiguration("debugpy.showPythonInlineValues")
				) {
